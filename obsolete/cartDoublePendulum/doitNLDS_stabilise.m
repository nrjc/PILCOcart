...
clear all; close all;
basename = 'CartDoubleNLDS_stabilise';
...
try
  ...
  addpath([rd 'base'],[rd 'util'],[rd 'util/tprod'],...
         [rd 'gp'],[rd 'gp/NIGP'],[rd 'control'],[rd 'loss'], [rd 'GPNLDS']);
catch
end
...
So = 0.1*[0.01 pi/180 pi/180 0.01/dt pi/180/dt pi/180/dt ].^2; % noise levels, 1cm, 1 degree
...
Do = length(dyno); Du = ctrl.U; Da = length(angi);
Di = length(dyni) + Du;
E = length(dyno); D = length(dyni) + ctrl.U;
dynmodel.fcn = @gpBase;                      % function for GP predictions
dynmodel.induce = zeros(50,0,1);                       % number inducing inputs
dynmodel.approxS = 0;                  % use full output covariance matrix
dynmodel.trainMean = 0;           % keep the GP mean fixed during training
[dynmodel.hyp(1:Do).m] = deal(zeros(Di,1)); [dynmodel.hyp.b] = deal(0);
for i=1:Do; dynmodel.hyp(i).m(i) = 1; end;                      % identity mean
dynmodel.hyp(1).m(4) = dt; dynmodel.hyp(2).m(5) = dt;  % Euler integration
dynmodel.hyp(3).m(6) = dt;
dynmodel.opt.length = -500;
dynmodel.opt.verbosity = 3;
dynmodel.opt.method = 'BFGS';
dynmodel.opt.fh = 6; figure(6);
...
cost.width = [1.2 0.5];                                 % cost function width
...
if usejava('awt'); 
    ...
    set(6,'windowstyle','docked','name','NLDS Optimisation')
