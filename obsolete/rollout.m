...
%
% Copyright (C) 2012-2014 by Carl Edward Rasmussen and Rowan McAllister,
% 2014-10-08
...
simi = sort([odei subi]);
nX = length(simi)+length(augi); U = ctrl.U; A = length(angi);

u = zeros(H, U); latent = zeros(H+1, nX+U);
L = zeros(1, H); next = zeros(1,length(simi)); x = zeros(H+1, nX+2*A);
...
for i = 1:H % ----------------------------------------------------- run ROLLOUT
  x(i,end-2*A+1:end) = gTrig(x(i,dyno)', zeros(length(dyno)), angi);  % trigaug
  
  % Apply policy ... or random actions ----------------------------------------
  if isfield(ctrl.policy, 'fcn')
    s.m = x(i,dyno)'; s.s = zeros(length(s.m));
    if i==1, s = ctrl.reset_filter(s); end             % reset filter if exists
    u(i,:) = ctrl.fcn(s, ctrl);
  else
    u(i,:) = ctrl.policy.maxU.*(2*rand(1,U)-1);
  end
  ...
  % Compute Cost --------------------------------------------------------------
  s.m = state(dyno)';
  if nargout > 2; L(i) = cost.fcn(cost, s); end
end
if verb; fprintf('\nTrial lasted %i steps\n',floor(H));  end

x(end,end-2*A+1:end) = gTrig(x(end,dyno)', zeros(length(dyno)), angi);
