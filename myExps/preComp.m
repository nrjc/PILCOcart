...
x = dyn.inputs; [N, D, pE] = size(x); E = size(dyn.hyp,2);
...
XmX=bsxfun(@minus,permute(x,[1,4,2,3]),permute(x,[4,1,2,3]));     % N-by-N-by-D-by-pE
%XmX2 = reshape(XmX.^2,N^2,D);
XmX2 = reshape(XmX.^2,N^2,D,pE);
%K = exp(bsxfun(@minus,2*[h.s],0.5*XmX2*iell));
XmX2iell = bsxfun(@times,XmX2,permute(iell,[3,1,2]));
K=exp(bsxfun(@minus,2*[h.s],0.5*reshape(sum(XmX2iell,2),N^2,E)));
...
if nargout > 1                                        % derivative calculations
  ...
  XmXiell=bsxfun(@times, XmX,permute(iell,[4,3,1,2])); %
  KdX=etprod('1243', K, '123', XmXiell,'1243'); %NxNxDxE
  iKKdX = etprod('12345', iK, '135', KdX, '3245'); % NxNxNxDxE
  iKdX = etprod('12345', iKKdX, '16345', iK, '625'); % NxNxNxDxE

  ddyn.Kdll = Kdll; ddyn.iKdll = iKdll; ddyn.iKdlpn = iKdlpn;
  ddyn.iKdlsf = iKdlsf; ddyn.iKdX = iKdX;
end