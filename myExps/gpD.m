...
h = gp.hyp; iK = gp.iK; beta = gp.beta; idx = gp.idx; Np = length(unwrap(idx));
...
for i=1:E     % compute predicted mean and inv(s) times input-output covariance
  ...
  dMdp(i,idx(i).inputs) = c*reshape(-P,1,[]);
  qbH = bsxfun(@times,permute(qb, [3 2 1]), H);                          %DxDxn
  GlqbGl = bsxfun(@times, permute(-P,[2 3 1]), permute(Gl, [3 2 1]));    %DxDxn
  dCdp(:,i,idx(i).inputs) = c*reshape(permute(qbH + GlqbGl,[1 3 2]), D, 1, []);
...
for i=1:E           % compute predictive covariance matrix, non-central moments
  ...
  for j=1:i
    ...
    %dSdp w.r.t inputs
    iiij = bsxfun(@plus,permute(ii,[1,3,2]),permute(ij,[3,1,2]));     % nxnxD
    ijiRs = reshape(reshape(iiij,n^2,D)*iRs,n,n,D);                     % nxnxD
    ijiRsiL = bsxfun(@times, ijiRs, permute(iL, [4 3 1 2]));            % nxnxDxE
    ijiRsiLii = bsxfun(@plus, ijiRsiL(:,:,:,i), permute(-ii,[3 1 2]));
    ijiRsiLij = bsxfun(@plus, ijiRsiL(:,:,:,j), permute(-ij,[3 1 2]));
    dLdxi = bsxfun(@times, L', ijiRsiLii);
    dLdxj = bsxfun(@times, L, ijiRsiLij);
    dbLbdxi = bsxfun(@times, bsxfun(@times,beta(:,i)', dLdxi), beta(:,j));
    dbLbdxj = bsxfun(@times, bsxfun(@times,beta(:,j)', dLdxj), beta(:,i));
    dSdp(i,j,idx(i).inputs)= reshape(t*sum(dbLbdxi,1), 1, []);
    dSdp(i,j,idx(j).inputs)= permute(dSdp(i,j,idx(j).inputs), [4 3 1 2]) ...
                                  + reshape(t*sum(dbLbdxj,1), 1, []);                  

    if i==j;
      diKLdii = -2*t*bsxfun(@times, iKL, ijiRsiLii);
      dSdp(i,j,idx(i).inputs)= permute(dSdp(i,j,idx(i).inputs), [4 3 1 2]) ...
                                        + reshape(sum(diKLdii,1), 1, []); 
    end
    ...
    if i==j; 
      ...
      fiKdXL = bsxfun(@times, gp.iKdX(:,:,:,:,i), L); %n-by-n-by-n-by-D
      iKdXL=permute(reshape(sum(fiKdXL, 1), n,n,D), [2 1 3]); %n-by-n-by-D
      dSdp(i,j,idx(i).inputs) = permute(dSdp(i,j,idx(i).inputs), [4 3 1 2]) ...
                                - 2*t*reshape(sum(iKdXL,2), 1, []);
    end
  end
