...
for i=1:length(dyno)          % plot the rollouts on top of predicted errorbars
  ...
  errorbar(22:22+length(stateM(dynoS(i),:))-1, stateM(dynoS(i),:), ...
                                   2*sqrt(squeeze(stateS(dynoS(i),dynoS(i),:))),'r');  
...
hold on;
errorbar(22:22+length([pred(j).action.m])-1, [pred(j).action.m],2*sqrt([pred(j).action.s]),'r');
...
for i=1:length(dyno) 
	subplot(floor(sqrt(nsp)), ceil(nsp/floor(sqrt(nsp))), i); hold on;
	for myi=1:5
		plot(0:length(data(j+J-myi).state(:,dynoS(i)))-1, data(j+J-myi).state(:,dynoS(i)), 'r');
	end
end
...
disptable([dyn.hyp.l], varNames, inputNames, '%0.5f')
