...
void applyRandomControl(int frame_interval, float curr_time, float dt, int &signal, float cart_pos, bool &return_mid)
{
  if (int(curr_time/dt)%frame_interval == 0 )
    {
      //signal = signal + ((rand()%100)/100.0 - 0.5)*1500;
      cout << "Cart pos is:" << cart_pos << endl; //signal + 
...
void applySimpleRandomControl(int frame_interval, float curr_time, float dt, float sens_pos, float sens_vel, int &signal, bool &return_mid){
  ...
  if (failsafe || int(curr_time/dt)%frame_interval == 0 )
    {
...
int main()
{
  ...
  sensor_angle_data.open("sensorn_rows_ang.txt");
  cam_anglev_data.open("cam_angv.txt");
  sensor_anglev_data.open("sensor_angv.txt");
  p_signal_data.open("p_signal.txt");
  ...
  while(true)
  {
    ...
    clock_gettime(CLOCK_MONOTONIC,  &t2);
  	...
  	//Scout << "Frame capture took " << after-before << " seconds" << endl;
    ...
    state(0) = p_signal;
    state(1) = cart_pos/LENP2*640/width;
    state(2) = (cart_pos-p_pos)/0.0333/LENP2*640/width; //BUG?! -pp_pos) //cart_velocity/LENP2
    state(3) = (angle-p_angle)/0.0333; //BUG?! -pp_angle) //anglev; 
    state(4) = sin_angle;
    state(5) = cos_angle;
    ...
    if(start){
      if (!end_return){
        if(!(syncro_counter%1)){
          if(init_rollout)
            applySimpleRandomControl(5,curr_time,dt,sens_pos,sens_vel,signal,return_mid);
          else
            applylearnedController(centers, weights, W, state, signal, return_mid);
        }
        //if(!(syncro_counter%1)) applyRandomControl(5,curr_time,dt,signal,cart_pos,return_mid);
      }
      else
    ...
    clock_gettime(CLOCK_MONOTONIC,  &t2);
	  after = (t2.tv_sec - t1.tv_sec) + (double) (t2.tv_nsec - t1.tv_nsec) * 1e-9;
	  //Scout << "Computation took " << after-before << " seconds" << endl;

    
    if (!end_return && start){
      // Write data to text files that can be read by MATLAB
      time_data << curr_time << " ";
      cart_position_data << state(1) << " ";
      cart_velocity_data << state(2) << " ";
      cam_angle_data << angle << " ";
      sensor_angle_data << sensor_angle << " ";
      cam_anglev_data << anglev << " ";
      sensor_anglev_data << sensor_anglev << " ";
      p_signal_data << p_signal << " ";
      signal_data << (float)MAX_U*signal/10000.0 << " ";
    ...
    // For calibrating
    cvLine(imgScribble,cvPoint(320,0), cvPoint(320,480),cvScalar(255,0,255), 2);
    cvLine(imgScribble,cvPoint(0,240), cvPoint(640,240),cvScalar(255,0,255), 2);

    // Beginning and the end
    // cvLine(imgScribble,cvPoint(320+65*3,0), cvPoint(320+65*3,480),cvScalar(255,0,255), 2);
    // cvLine(imgScribble,cvPoint(320-65*3,0), cvPoint(320-65*3,480),cvScalar(255,0,255), 2);
