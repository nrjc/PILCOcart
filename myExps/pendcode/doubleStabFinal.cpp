...
#define PINKTHRESH  160, 90, 140, 180, 256, 256
...
int main(int argc, char *argv[])
{
    ...
    float pp_cos = 1; //Usually I will start the pendulum at top position and cos is 1 there
    float pp_sin = 0;
    float last_cos = 1; //Usually I will start the pendulum at top position and cos is 1 there
    float last_sin = 0;

    float pp_cos_2 = 1; //Usually I will start the pendulum at top position and cos is 1 there
    float pp_sin_2 = 0;
    float last_cos_2 = 1; //Usually I will start the pendulum at top position and cos is 1 there
    float last_sin_2 = 0;
    ...
    while(true)
    {
        ...
        if (!start && curr_time > (initiation_time+0.25)) {
            cout << "RELEASE!" << endl;
        }
        ...
        if (curr_time > 4.0 + initiation_time && !end_return || (sens_pos_pred<-SIDE_LIMIT || sens_pos_pred>SIDE_LIMIT))
        {
            S626_WriteDAC (0, 0, 0);
            end_return = 1;
        }
