function [M, S, C, dMdm, dSdm, dCdm, dMds, dSds, dCds, dMdp, dSdp, dCdp] = conGaussdfw(policy, m, s)
...
if isfield(policy,'ll'); ll = policy.ll; fxdll = 1; 
else ll = policy.p.ll; fxdll = 0; end
w = policy.p.w;
...
dMdc = zeros(E,n,D); dSdc = zeros(E,E,n,D); dCdc = zeros(D,E,n,D);
...
for i = 1:E
  ...
  % ----------------------------------------- derivatives w.r.t RBF centres
    dMdc(i,:,:) = -c*tlb;
    for d = 1:D
      dCdc(:,i,:,d) = c*(iRR(:,d)*lb' - bsxfun(@times,tt,tlb(:,d))');
    end
...
for i = 1:E
  ...
  for j = 1:i
    ...
    if i==j
      ...
      if ~fxdll; dSdll(i,i,:,i) = reshape(t*dSdll(i,i,:,i),D,1)' + tdX*ssA; end
    else
      dSdw(i,j,:,i) = w(:,j)'*L'*t;
      dSdw(i,j,:,j) = w(:,i)'*L *t;
      if ~fxdll; dSdll(i,j,:,i) = reshape(t*dSdll(i,j,:,i),D,1)' + tdXi*ssA;
        dSdll(i,j,:,j) = reshape(t*dSdll(i,j,:,j),D,1)' + tdXj*ssA; end
    end
...
dMdp = dMdw(:,:); dSdp = reshape(dSdw,E*E,n*E); dCdp = reshape(dCdw,D*E,n*E);

if ~fxdll
    dMdp = [dMdll(:,:) dMdp];
    dSdp = [reshape(dSdll,E*E,D*E) dSdp];
    dCdp = [reshape(dCdll,D*E,D*E) dCdp];
end
