% Compute moments of the 7th order Fourier series fit to a sigmoid
...
% the moments.
%
% Copyright (C) 2015 by Carl Edward Rasmussen, 2015-03-12.
% Edited by Rowan 2015-07-23

function [M, S, C, dMdm, dSdm, dCdm, dMdv, dSdv, dCdv] = gSat7(m, v, i, e)
...
d = length(m); I = length(i); i = i(:)'; f = [1; 3; 5; 7]/2.683; F = length(f); 
...
if nargout < 4                                          % derivatives required?
  [M2, S2, C2] = gSin(ma, va, d+1:d+F*I, e*[1.2154, 0.2894, 0.0931, 0.0191]);
else
  [M2, S2, C2, Mdma, Sdma, Cdma, Mdva, Sdva, Cdva] ...
                 = gSin(ma, va, d+1:d+F*I, e*[1.2154, 0.2894, 0.0931, 0.0191]);
end

R = [eye(I) eye(I) eye(I) eye(I)];
M = R*M2;                                                                % mean
...
if nargout > 3                                        % derivatives if required
  ...
  dSdm = kron(R,R)*Sdma*madm;  dSdv = symmetrised(kron(R,R)*Sdva*vadv,1);
  dCdm = kron(R,P')*Cdma*madm; dCdv = kron(R,P')*Cdva*vadv;
  dSdv = symmetrised(dSdv,1);
end
