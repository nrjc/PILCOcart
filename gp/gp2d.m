...
%   hyp     log-hyper-parameters ( P = D+2 )                        [ P  x  E ]
...
for i = 1:E
  ...
  for j = 1:i % if i==j: diagonal elements of S; see Marc's thesis around eq. (2.26)
    ...
    if rcond(R) < 1e-15; warning('gp2d: R-matrix ill-conditioned'); end
    iR = R\eye(D);
    ...
    for d = 1:D
      %       Z(:,d) = exp(-2*X(d,i))*sum(A.*bsxfun(@plus,zzi(:,d)-inp(:,d),zzj(:,d)'),2)...
      %         +exp(-2*X(d,j))*sum(A.*bsxfun(@plus,zzi(:,d),zzj(:,d)'-inp(:,d)'),1)';
