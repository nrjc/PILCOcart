...
% Policy
policy.fcn = @(policy,m,s)conCat(@contrig,@gSat,policy,m,s);
policy.maxU = maxU;                      % max. amplitude of control
policy.p=struct([]);
nc = 30; %number of bases
policy.p(1).w = 0.1*randn(nc, D+2);
policy.p(1).a = 0.1*randn(nc, E+2);
policy.p(1).b = 0.1*randn(nc, 1);
policy.opt = ...
