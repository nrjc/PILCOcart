...
% 
% Andrew McHutchon, September 2012. Edited Rowan McAllister 2014-11-07
...
for n=1:Npol                                 % Loop over each of the sub-polices
    ...
    if isfield(policy, 't'), pol.t = policy.t; end;
...
if strcmpi(policy.addCtrlNoise, 'on')
    % Add activation noise to S, and accordingly add random perturbation to M
    actNoise = (1./(1+exp(-M))) .* M.^2 * policy.ctrlNoise;
    S = S + diag(actNoise) + diag(policy.ctrlNoise * diag(S));
    M = M + chol(S + diag(1e-12 * ones(E,1)))*randn(E,1);
end
