...
for j = 1:N
  ...
  % Save cost, state and path plots for each trial (png and fig)
  h = get(0,'children');
  for i=1:3
    saveas(h(i), [basename num2str(j) '_figure' num2str(length(h) + 1 - i)], 'fig');
    saveas(h(i), [basename num2str(j) '_figure' num2str(length(h) + 1 - i)], 'png');
  end
end

% Save minimisation graph
saveas(h(4), [basename num2str(j) '_figure' num2str(length(h) + 1 - 4)], 'fig');
saveas(h(4), [basename num2str(j) '_figure' num2str(length(h) + 1 - 4)], 'png');
toc