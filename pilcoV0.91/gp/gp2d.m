...
function [M, S, V, dMdm, dSdm, dVdm, dMds, dSds, dVds, dMdp, dSdp, dVdp] ...
                                                      = gp2d(gpmodel, x, m, s)
%% Code
persistent K oldh oldb
input = x;  beta = gpmodel.beta; iK = gpmodel.iK; h = gpmodel.hyp;
[n,E] = size(beta); D = size(input,2);

if nargout < 4; [M, S, V] = gp2(gpmodel, m, s); return; end

if isempty(K) || ~isequal(unwrap(h),oldh) || ~isequal(beta,oldb)
  K = zeros(n,n,E);
  for i=1:E                                            % compute K and inv(K)
    inp = bsxfun(@times,x,exp(-h(i).l'));
    K(:,:,i) = exp(2*h(i).s-maha(inp,inp)/2);
  end
  oldh = unwrap(h); oldb = beta;
end
...
for i = 1:E
  ...
  K2 = K(:,:,i)+exp(2*h(i).n)*eye(n);                         % K + sigma^2*I
  inp2 = bsxfun(@rdivide,input,exp(h(i).l'));
  ii = bsxfun(@rdivide,input,exp(2*h(i).l'));
  R = s+diag(exp(2*h(i).l));
  L = diag(exp(-h(i).l));
  ...
  c = exp(2*h(i).s)/sqrt(det(R))*exp(sum(h(i).l));
  detdX = diag(bsxfun(@times,det(R)*iR',2.*exp(2.*h(i).l)));    % d(det R)/dX
  cdX = -0.5*c/det(R).*detdX'+ c.*ones(1,D);       % derivs w.r.t length-scales
  dldX = bsxfun(@times,l,bsxfun(@times,t,2.*exp(2*h(i).l')).*t./2);
  
  M(i) = sum(lb)*c;                                            % predicted mean
  
  iK2beta = K2\beta(:,i);
  dMds(i,:,:) = c*t'*tlb/2-iR*M(i)/2;
  dMdX(i,D+2,i) = -c*sum(l.*(2*exp(2*h(i).n)*iK2beta));           % OK
  dMdX(i,D+1,i) = -dMdX(i,(i-1)*(D+2)+D+2);
  
  dVdX(:,i,D+2,i) = -((l.*(2*exp(2*h(i).n)*iK2beta))'*t*c)';
  ...
  for d = 1:D
    ...
    dtdX = inp*(-bsxfun(@times,iR(:,d),2.*exp(2*h(i).l(d))*iR(d,:)));
    dlbt = lb'*dtdX + dlb'*t;
  ...
  v = bsxfun(@rdivide,inp,exp(h(i).l'));
  k(:,i) = 2*h(i).s-sum(v.*v,2)/2;
...
for i = 1:E
  K2 = K(:,:,i)+exp(2*h(i).n)*eye(n);
  ii = bsxfun(@rdivide,inp,exp(2*h(i).l'));
  
  for j = 1:i % if i==j: diagonal elements of S; see Marc's thesis around eq. (2.26)
    R = s*diag(exp(-2*h(i).l)+exp(-2*h(j).l))+eye(D); t = 1./sqrt(det(R));
    if rcond(R) < 1e-15; fprintf('R-matrix in gp2d ill-conditioned'); keyboard; end
    iR = R\eye(D);
    ij = bsxfun(@rdivide,inp,exp(2*h(j).l'));
    ...
    tdX  = -0.5*t*sum(iR'.*bsxfun(@times,s,-2*exp(-2*h(i).l')-2*exp(-2*h(i).l')));
    tdXi = -0.5*t*sum(iR'.*bsxfun(@times,s,-2*exp(-2*h(i).l')));
    tdXj = -0.5*t*sum(iR'.*bsxfun(@times,s,-2*exp(-2*h(j).l')));
    ...
    for d = 1:D
      ...
      Z(:,d) = exp(-2*h(i).l(d))*(A*zzj(:,d) + sum(A,2).*(zzi(:,d) - inp(:,d)))...
        + exp(-2*h(j).l(d))*((zzi(:,d))'*A + sum(A,1).*(zzj(:,d) - inp(:,d))')';
      Q = bsxfun(@minus,inp(:,d),inp(:,d)');
      B = K(:,:,i).*Q;
      Z(:,d) = Z(:,d)+exp(-2*h(i).l(d))*(B*beta(:,i).*bLiKj+beta(:,i).*(B*bLiKj));
      
      if i~=j; B = K(:,:,j).*Q; end
      
      Z(:,d) = Z(:,d)+exp(-2*h(j).l(d))*(bLiKi.*(B*beta(:,j))+B*bLiKi.*beta(:,j));
      ...
      if i==j
        RTi =  bsxfun(@times,s,(-2*exp(-2*h(i).l')-2*exp(-2*h(j).l')));
        diRi = -R\bsxfun(@times,RTi(:,d),iR(d,:));
      else
        RTi = bsxfun(@times,s,-2*exp(-2*h(i).l'));
        RTj = bsxfun(@times,s,-2*exp(-2*h(j).l'));
        diRi = -R\bsxfun(@times,RTi(:,d),iR(d,:));
    ...
    if i==j
      ...
      dSdX(i,i,D+2,i) = 2*exp(2*h(i).n)*t*(-sum(beta(:,i).*bLiKi)-sum(beta(:,i).*bLiKi));
    else
      ...
      dSdX(i,j,D+2,i) = 2*exp(2*h(i).n)*t*(-beta(:,i)'*bLiKj);
      dSdX(i,j,D+2,j) = 2*exp(2*h(j).n)*t*(-beta(:,j)'*bLiKi);
    end
    
    dSdm(i,j,:) = r - M(i)*dMdm(j,:)-M(j)*dMdm(i,:); dSdm(j,i,:) = dSdm(i,j,:);
    T = (t*T-S(i,j)*diag(exp(-2*h(i).l)+exp(-2*h(j).l))/R)/2;
    ...
    if i==j
      ...
      dSdt(i,j,:,i) = (beta(:,j)'*L')/(K2)*t ...
        - dMdt(i,:,i)*M(j);
      dSdt(i,j,:,j) = beta(:,i)'*L/(K(:,:,j)+exp(2*h(j).n)*eye(n))*t ...
        - dMdt(j,:,j)*M(i);
      dSdt(j,i,:,:) = dSdt(i,j,:,:);
