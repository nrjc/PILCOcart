...
%   targets   n by E matrix of training targets
%
...
for i=1:E                                            % compute K and inv(K)
  ...
  if isfield(gpmodel,'targets');
    y = gpmodel.targets(:,i) - x*h(i).m - h(i).b;
...
for i=1:E
  ...
  if isfield(gpmodel,'targets')
    y = gpmodel.targets(:,i) - input*h(i).m - h(i).b;
    beta(:,i) = iK(:,:,i)*y;
  end
...
gpmodel.iK = iK2;
if isfield(gpmodel,'targets'); gpmodel.beta = beta; end
